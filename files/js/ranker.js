...
async function slotsWrite(slots) {
  // normalize to exactly 3
  const normalized = Array.isArray(slots) ? slots.slice(0, 3) : [];
  while (normalized.length < 3) normalized.push(null);

  // if logged in, prefer authoritative cloud overwrite
  if (window.PokeRankrAuth?.isLoggedIn?.() && window.PokeRankrSync?.overwriteSaveSlots) {
    try {
      await window.PokeRankrSync.overwriteSaveSlots(normalized);
